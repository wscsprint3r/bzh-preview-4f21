 ...
 * `toMarkdown` already calls this function, so **callers pass raw HTML to
 * `toMarkdown` and never call `stripPreamble` themselves first** - calling
 * both strips twice.
 *
 * Measured against all 71 real published posts and pages, 2026-09-17
 * (`migration/check-preamble.mjs`, run by hand - it needs Docker and the
 ...
 * not of the regexes; `check-preamble.mjs` checks them on demand, not
...
// <script>, 2 <style> (`who-we-has`, `church`, both outside this phase's
// nine pages).
 ...
 * `stripPreamble` + Turndown + `normalize`: no tab and no run of three
 ...
 *    afterwards, when `normalize` runs on Turndown's output. `.trim()`
...
/**
 * HTML in, Markdown out - Turndown converts, THEN `normalize` runs.
 *
 * That order is load-bearing, not a stylistic choice. `post_content` is
 * WordPress HTML: an entity like `&nbsp;` or `&#160;` is plain ASCII BUILT_TEXT to
 * `normalize` - an ampersand and some letters or digits, nothing it
 ...
 * U+00A0. Normalising before conversion would run `normalize` over a
 ...
 * in total, against the 459 literal non-breaking spaces `normalize` was
 * measured removing - a 32x regression, invisible to a reader because
 * U+00A0 renders as an ordinary space.
 *
 * `name` is optional and exists only for the two prints below: keeping a
 ...
 * quiet one.
 */
export function toMarkdown(html, name) {
  const label = name ?? '(unnamed document)';
  if (/<table\b/i.test(html)) {
    process.stdout.write(`toMarkdown: ${label} keeps a <table> as raw HTML\n`);
  }
  if (/<script\b/i.test(html) || /<style\b/i.test(html)) {
    process.stdout.write(`toMarkdown: ${label} removes <script>/<style>\n`);
  }
