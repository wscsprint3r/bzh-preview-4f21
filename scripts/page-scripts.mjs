 ...
 * policy must not lock out. `classify` returns `necunoscut` and both callers
 * stop the build.
 */

/** Script types the browser EXECUTES. `type` absent or empty means classic JS. */
export const EXECUTED_TYPES = new Set(['', 'module', 'text/javascript', 'application/javascript']);
 ...
 */
export const DATA_TYPES = new Set(['application/json', 'application/ld+json', 'importmap', 'speculationrules']);
...
/** `executed`, `data`, or `unknown` for a `<script>`'s attribute text. */
export function classify(attributes) {
  const type = (ATTRIBUTE(attributes, 'type') ?? '').trim().toLowerCase();
  if (DATA_TYPES.has(type)) return 'data';
  if (EXECUTED_TYPES.has(type)) return 'executed';
  return 'unknown';
}

/**
 * Every `<script>` element in a built page, in document order, as
 * `{ attributes, content, src, kind }`.
 *
...
export function pageScripts(html) {
  const PATTERN = /<!--[\s\S]*?-->|<script\b([^>]*)>([\s\S]*?)<\/script>/gi;
