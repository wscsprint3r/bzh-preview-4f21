 ...
 *     copy of `strictKeys` rather than importing one. The import there is fixed
 ...
 *   in `content-schema.test.ts`, which spawns a real `node`.
 ...
 * different words: `missing` is what a volunteer reads when the line is gone
 * altogether (Zod reports that as a type error, since the value is `undefined`),
 * and `empty` is what they read when the line is there but holds only spaces.
 *
 ...
 */
const nonEmptyText = (missing: string, empty: string) =>
  z
    .string({ error: (issue) => (issue.code === 'invalid_type' ? missing : undefined) })
    .trim()
    .min(1, { message: empty });
 ...
 *
 * `dateParts` rather than a regex, and for the reason it was written: a regex
 ...
 * actively misleading. `date` missing, `date: null` and `date: 20251105` are all
...
export const articleSchema = z
  .strictObject(
    {
      title: nonEmptyText('Articolul trebuie să aibă un titlu.', 'Titlul nu poate fi gol.'),
      date: realDate,
      /*
       ...
       * rather than by the code. The list comes from `CATEGORIES` so the message
       * cannot fall behind the set it describes.
       */
      category: z.enum(CATEGORIES, {
        error: (issue) =>
          issue.input === undefined
            ? `Articolul trebuie să aibă o categorie: ${CATEGORIES.join(' sau ')}.`
            : `Categoria poate fi doar ${CATEGORIES.join(' sau ')}.`,
      }),
      author: z
        .string({
          error: (issue) =>
...
export const pageSchema = z
  .strictObject(
    {
      title: nonEmptyText('Pagina trebuie să aibă un titlu.', 'Titlul paginii nu poate fi gol.'),
      /*
...
export const settingsSchema = z
  ...
  /*
   * Checked across EVERY field rather than on `email` and `phone` by name.
    ...
    for (const [key, value] of Object.entries(v)) {
      if (typeof value === 'string' && DEMO.includes(value.trim())) {
        ctx.addIssue({
          ...
          path: [key],
          message:
            `Valoarea „${value}” este o rămășiță demo de pe situl vechi, ` +
