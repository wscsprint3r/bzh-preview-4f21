 ...
 *
 *   - `diacritics.itest.ts` reads `dist/`, the built output, which is where a
 *     `feast:` field a volunteer typed becomes text a parishioner sees. It
 ...
 *   - `diacritics-sources.test.ts` reads every TRACKED file, which is where a
...
export const CEDILLAS = [0x015e, 0x015f, 0x0162, 0x0163] as const;

/** The Romanian comma-below forms they are mistaken for. */
export const COMMA_BELOW = [0x0218, 0x0219, 0x021a, 0x021b] as const;
 ...
 * means clean.
 */
export function cedillasIn(text: string): string[] {
  const found: string[] = [];
...
export function hasCommaBelow(text: string): boolean {
  for (let i = 0; i < text.length; i += 1) {
    const cp = text.codePointAt(i);
    if (cp !== undefined && (COMMA_BELOW as readonly number[]).includes(cp)) return true;
  }
  return false;
}
