 ...
 * `todayInZurich`/`timeInZurich` in `week.ts`, and callers pass their results in.
 ...
 *
 * - `time` is already canonical `HH:MM`. `minutes()` still parses `9:30`, because
...
import { dateParts, dayHeading } from './date-ro';
 ...
 * a start time.
 *
 * `Service` is that plus what the service is called; the projection the homepage
 ...
 * this reads anything beyond `time` - so none of them asks for anything more.
 ...
 */
export type WithTime = { time: string };

/**
 * The part of a day that deciding "what happens next" actually reads.
 *
 * `nextService` takes this rather than a whole `ServiceDay` because from Task
 ...
 */
export type ScheduleDay<S extends WithTime = Service> = Pick<
  ServiceDay,
  'date' | 'cancelled' | 'location'
> & { services: S[] };

/** A service as the card shows it: a time, and the name `serviceLabel` gave it. */
export type CardService = WithTime & { name: string };
 ...
 *
 * Every word the card displays arrives ALREADY RENDERED - `title` is "Duminică,
 * 20 septembrie" and `name` is what `serviceLabel` returned, both written by
 * the server that was going to write them anyway. The browser is sent the
 * DECISION and not the rendering: which service is next stays `nextService`
 * and `servicesAtSameTime`, the same functions on both sides, so the two answers
 * can differ only because the clock moved. A string copied cannot be formatted
 * differently from the one it was copied from, and `MONTH_NAMES` never has to
 * cross the wire to say "septembrie" twice.
 */
export type CardDay = ScheduleDay<CardService> & { title: string };
 ...
 * use (`z.date >= today`, `s.monday >= monday`). One comparison semantics per date
 ...
 *
 * This is the single place a service name is rendered - `DayRow`, the week
 ...
 *
 * The `|| 'Slujbă'` fallback should be unreachable: `serviceSchema` refuses an
 * `Altceva` without a non-blank `detail`. It stays because the alternative to
...
/**
 * The full stop a composed sentence needs after `value`, or `''` when the
 * value already brings its own.
 *
 * Two pages wrap `location` in a sentence - "Slujbele acestei zile au loc la X."
 ...
 *
 * Fixed HERE, at presentation time, and deliberately not in `daySchema` where
 ...
 * stores rather than prose this site is composing. Canonicalising `time` to
 ...
 * reason `servicesInOrder` exists: two components composing the same sentence
 ...
 */
export function fullStop(value: string): string {
  return /[.…]$/.test(value) ? '' : '.';
}
 ...
 * service sort came to disagree; see `servicesInOrder`.
 *
 ...
 */
export function romanianList(name: string[]): string {
  if (name.length === 0) return '';
  if (name.length === 1) return name[0];
  return `${name.slice(0, -1).join(', ')} și ${name[name.length - 1]}`;
}

/**
 * A day's services in the order they happen.
 *
 * `daySchema` neither sorts `services` nor requires them sorted - YAML keeps the
 ...
 * callers: `nextService` (which has always sorted), `ics.ts` (which has
 * always sorted) and `groupIntoWeeks`, through which both pages read the
 ...
 * date comparison would - see `compareDates`.
 *
 ...
 * module cannot see.
 */
export function servicesInOrder<S extends WithTime>(services: S[]): S[] {
  return [...services].sort((a, b) => minutes(a.time) - minutes(b.time));
}
 ...
 * order they happen by `servicesInOrder`. Nothing is de-duplicated and no
 ...
 * because `DayRow` on `/program/` and `WeekBand` on the homepage are
  ...
  return [...buckets.values()]
    .map((group) => {
      const sorted = [...group]
        ...
        .map((z) => ({ ...z, services: servicesInOrder(z.services) }));
     ...
     * was: "sorting by `key` would break where 2026-W53 runs into 2027-01-03"
     ...
     * `weekKey` zero-pads the week number, which makes `YYYY-Www` a
     * strictly monotone function of the Monday. Sorting by `a.key` here is a
     ...
     * `compareDates` - this module's single comparison semantics for date strings -
     * instead of on the key's SPELLING. The key's monotonicity is a separate
     * property, asserted where it is actually relied upon: `pickWeek` in
     ...
     */
    .sort((a, b) => compareDates(a.monday, b.monday));
}

/**
 * The first service at or after `today time`, or null if the schedule is spent.
 *
 ...
 *
 * Returns a `Service` widened with its date, so the caller can hand the result
 * straight to `serviceLabel`. Among services that start at the same minute,
 ...
 *
 * Throws on an `today` that is not a real `YYYY-MM-DD` date. `time` is not
 * validated: a malformed one makes `now` NaN, which skips the rest of today
 * and moves on - wrong, but not confidently wrong in the way a bad `today` is.
 */
export function nextService<S extends WithTime>(
  days: ScheduleDay<S>[],
  today: string,
  time: string,
): (S & { date: string }) | null {
  // The other two functions here validate their date for free, by handing it to
  // weekKey/weekStart. This one only ever string-compares, so
  ...
  // below every stored date, so every day clears `z.date >= today` and the
...
/**
 * Every service in `services` that starts at the same minute as `time`.
 *
 * The companion to `nextService`, and the reason it needs one: that
 ...
 * Confession runs during vespers, which is why `daySchema` permits `17:00
 ...
 *
 * Compared as minutes, like everything else in this module: `time` is already
 ...
 *
 * Order is the caller's: `services` is not sorted here, so the editor's order
 * survives, exactly as it does through `groupIntoWeeks`.
 */
export function servicesAtSameTime<S extends WithTime>(services: S[], time: string): S[] {
  const target = minutes(time);
  return services.filter((s) => minutes(s.time) === target);
}

/**
 * The week containing `today` plus the following `count - 1` weeks that have
 * entries.
 *
 ...
 *
 * `Math.max` guards a non-positive `count`: `slice(0, -1)` would otherwise return
...
/* -------------------------------------------------------------------------- *
 * THE HOMEPAGE'S DATE ISLAND, AND WHY IT IS BOUNDED BY A COUNT.
 *
 ...
 * It used to carry EVERY future day - `days.filter((z) => z.date >= today)` - which
 ...
 * SEPARATE, CACHEABLE, GZIPPED `/program/data.json`. Deviation 1 dropped that
 ...
 * future service in the island and HIDES the card - `WeekPicker`'s one
 ...
 * days were discarded, while `nextService` discards them afterwards. So
 * `ISLAND_DAYS` consecutive cancelled published days emptied the island of
 ...
 * than the reason: `upcomingWeeks` counts weeks WITH ENTRIES and can span
...
export const ISLAND_DAYS = 40;

/**
 * The days the client script can actually use, as the homepage embeds them:
 * from `today` forward, in order, at most `count` of them.
 *
 * A projection rather than the days themselves - a `feast` or a `note` would be
 * bytes on every homepage for fields the card never renders.
 *
 * CANCELLED DAYS ARE DISCARDED BEFORE THE SLICE, not after, so `count` bounds days
 ...
 * cancelled Liturgy instead of announcing one. `ScheduleDay` requires the field,
 ...
 * render it anyway: `serviceLabel` owns the `Altceva` escape hatch, and the
 * card must never be the one place that word reaches a page. The browser gets the
 * DECISION - `nextService` and `servicesAtSameTime`, the same functions on
 * both sides - and never a second copy of the rendering.
 */
export function scheduleForIsland(
  days: ServiceDay[],
  today: string,
  count: number = ISLAND_DAYS,
): CardDay[] {
  // Validated for the same reason `nextService` validates it: a malformed
  // `today` sorts below every stored date, so every day would clear the filter and
  ...
  return days
    ...
    .map((z) => ({
      ...
      location: z.location,
      services: z.services.map((s) => ({ time: s.time, name: serviceLabel(s) })),
      title: dayHeading(z.date),
    }));
}
